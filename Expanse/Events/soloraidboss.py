...
b1 = types.InlineKeyboardButton(text='Установить оповещение', callback_data='expanse_setsolorb')
b2 = types.InlineKeyboardButton(text='Убрать оповещение', callback_data='expanse_removesolorb')

inline_soloraidboss_buttons.add(b1, b2)


# SOLO RAID BOSS SETTINGS
@dp.message_handler(commands=['expanse_soloraidboss'])
async def expanse_about_soloraidboss(message: types.Message):
...
@dp.callback_query_handler(filters.Text(contains='expanse_setsolorb'))
async def expanse_set_soloraidboss(callback_query: types.CallbackQuery):
    session = Session()
...
@dp.callback_query_handler(filters.Text(contains='expanse_removesolorb'))
async def expanse_remove_soloraidboss(callback_query: types.CallbackQuery):
    session = Session()
...
async def expanse_soloraidboss_notification_wrapper():
    session = Session()
    users = session.query(User).all()
    for user in users:
        setting = session.query(Expanse).filter_by(id_user=user.telegram_id).first()
        if setting.soloraidboss is True and setting.fulltime is True:
            await expanse_soloraidboss_notification(user)
        elif setting.soloraidboss is True and setting.fulltime is False:
            await expanse_soloraidboss_notification_hardwork(user)
    session.close()


async def expanse_soloraidboss_notification(user: User):
...
async def expanse_soloraidboss_notification_hardwork(user: User):
    now = datetime.now().strftime('%H:%M')
